...
void deserialise_row(void* destination, Row* source) {
    memcpy(&(source->id), destination + ID_OFFSET, ID_SIZE);
    memcpy(&(source->username), destination + USERNAME_OFFSET, USERNAME_SIZE);
    memcpy(&(source->email), destination + EMAIL_OFFSET, EMAIL_SIZE);
}
